    ...
    <div class="gva-table-box">
      <div class="gva-btn-list">
        ...
        <el-button icon="delete" style="margin-left: 10px;" 
          @click="()=>{onDelete(7)}">删除7以前的</el-button>
        <el-button icon="delete" style="margin-left: 10px;"
          @click="()=>{onDelete(30)}">删除30天以前的</el-button>
      </div>
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="rawMsg" prop="msgId" width="120">
          <template #default="scope"><a :href="'/#/layout/admin/rawmsg?ID='+scope.row.msgId">{{ scope.row.msgId
              }}</a></template>
        </el-table-column>

        <el-table-column align="left" label="价格" prop="price" width="120" />
        <el-table-column align="left" label="价格波动1 %" prop="priceTestWave1" width="120">
          <template #default="scope">{{percentRatio(scope.row.priceTestWave1)}}</template>
        </el-table-column>
        <el-table-column align="left" label="价格波动2  %" prop="priceTestWave2" width="120">
          <template #default="scope">{{percentRatio(scope.row.priceTestWave2)}}
          </template>
        </el-table-column>
        <el-table-column align="left" label="价格波动2 %" prop="priceTestWave3" width="120">
          <template #default="scope">{{percentRatio(scope.row.priceTestWave3)}}
          </template>
        </el-table-column>
        <el-table-column align="left" label="价格1" prop="priceTest1" width="120" />
        <el-table-column align="left" label="价格1" prop="priceTest2" width="120" />
        <el-table-column align="left" label="价格1" prop="priceTest3" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
          <template #default="scope">
            ...
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateCoinsFunc(scope.row)">变更</el-button>
...
<script>
  //格式华百分比
  const percentRatio = (v) => {
    let percV = parseFloat(v) * 100
    // console.log('percentRatio1',percV)
    percV = Number(percV).toLocaleString('en-US', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 6
    });
    return `${percV}%`
    // console.log('percentRatio2',percV)
  }
...
<script setup>
  ...
  const onDelete = async (day) => {
    ...
    ElMessageBox.confirm('确定要删除吗?', '提示', {
      ...
      if (day == 7) {
        const res = await deleteCoinsByDay({ day })
        if (res.code === 0) {
          ElMessage({
            type: 'success',
            message: '删除成功'
